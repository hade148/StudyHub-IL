...
const getFileType = (filePath: string): string => {
  const ext = filePath.split('.').pop()?.toUpperCase() || 'PDF';
  return ext;
};

// Transform API data to display format
const transformApiData = (apiData: ApiSummary[]): SummaryDisplayData[] => {
  return apiData.map((summary) => ({
    id: summary.id,
    title: summary.title,
    ...
    downloads: 0, // API doesn't track downloads yet
    comments: summary._count.comments,
    fileType: getFileType(summary.filePath),
    fileSize: '', // API doesn't have file size
    pages: 0, // API doesn't have page count
    description: summary.description || '',
    uploader: summary.uploadedBy.fullName,
    uploadDate: formatRelativeDate(summary.uploadDate),
    tags: [], // Tags not implemented in API
...
export function SummariesPage({ onNavigateHome, onNavigateUpload, onNavigateSummary }: SummariesPageProps) {
  ...
  const [summariesData, setSummariesData] = useState<SummaryDisplayData[]>([]);
  ...
  // Filter and sort summaries (client-side filtering for course and fileType)
  const filteredAndSortedSummaries = useMemo(() => {
    ...
    return result;
  }, [summariesData, courseFilter, fileTypeFilter, sortBy]);
